...
class NicPlugin(BasePlugin):
    ...
    def linux_interfaces(self):
        ...
        ip_path = 'ip'
        if ip_path:
            stdin1, stdout1, stderr1 = self.ssh.exec_command("ip link show")
            stdin2, stdout2, stderr2 = self.ssh.exec_command("ip addr show")
            cmd1 = stdout1.read().decode('gbk').strip()
            cmd2 = stdout2.read().decode('gbk').strip()
